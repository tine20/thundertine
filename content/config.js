...
var config = {
	...
	write : function() {
		devTools.enter("config", "write");
		/*
		 * all preferences which are not subject to change with every sync are
		 * in the thunderbird preferences and can only be edited within options
		 * dialog
		 */
		...
		for ( var i = 0; i < this.folderIds.length; i++) {
			...
			folders.lastChild.lastChild.appendChild(doc
					.createTextNode(this.folderIds[i]));
			folders.lastChild.appendChild(doc.createElement('name'));
			folders.lastChild.lastChild.appendChild(doc
					.createTextNode(this.folderNames[i]));
			folders.lastChild.appendChild(doc.createElement('type'));
			folders.lastChild.lastChild.appendChild(doc
					.createTextNode(this.folderTypes[i]));
		}
		...
		devTools.leave("config", "write");
	},

	read : function() {
		devTools.enter("config", "read");
		...
		for ( var i = 0; i < doc.firstChild.childNodes.length; i++) {
			if (doc.firstChild.childNodes[i].nodeName == 'folders') {
				...
				if (doc.firstChild.childNodes[i].firstChild != null)
					this['managedCards'] = doc.firstChild.childNodes[i].firstChild.nodeValue
							.split(',');
				else
		...
		if (this.folderIds.indexOf(this.contactsRemoteFolder) >= 0
				&& ab.indexOf(config.contactsLocalFolder) >= 0
				&& config.user != '' && config.deviceId != ''
	...
	getPwd : function() {
		devTools.enter("config", "getPwd");

		if (config.pwdCache == '') {
			try {
				devTools.writeMsg("config", "getPwd", "opening password manager");
				// data in thunderbird config
				var prefs = Components.classes["@mozilla.org/preferences-service;1"]
						.getService(Components.interfaces.nsIPrefService);
				prefs = prefs.getBranch("extensions.ttine.");
	...
	setPwd : function(pwd) {
		devTools.enter("config", "setPwd");
		config.pwdCache = pwd;
		devTools.leave("config", "setPwd");
	},
